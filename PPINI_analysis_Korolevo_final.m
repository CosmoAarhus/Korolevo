...
site = ' ';
name = 'mc_sink_Korolevo_constrained68_maxpluck1';    % Library name 
file = 'Korolevo_final.xlsx';      % Excel sheet with sample information
...
leg_str = {'1 in 1000 of simulations';'Accepted simulations';'^{26}Al/^{10}Be ratio '};
...
for j = 1:numel(k)
    ...
    title(names(i))
    ax = gca;
    xlabel('Burial age [Ma]')
    ylabel('Acc. rate [g/cm2/Myr]')
...
% ax.YLim = [0; 4e4*7];
axis([0 4e6 0 4e7])
xlabel('^1^0Be [10^6 at/g]')
ylabel('^2^6Al [10^6 at/g]')
set(gca,'xtick',[0 0.5e6 1e6 1.5e6 2e6 2.5e6 3e6 3.5e6 4e6]);
set(gca,'xticklabels',[0.0 0.5 1.0 1.5 2.0 2.5 3.0 3.5 4.0]);
set(gca,'ytick',[0 5e6 10e6 15e6 20e6 25e6 30e6 35e6 40e6]);
set(gca,'yticklabels',[0 5 10 15 20 25 30 35 40]);
uistack(leg2(3),'top')
%keyboard
